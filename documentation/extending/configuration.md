...
Configure plugin system behavior, resource limits, and circuit breakers in `modula.config.json`.
...
Master switch for the entire plugin system. When `false`, no plugins load, no routes register, and no hooks fire. The plugin directory is not scanned.
...
Total VM pool size per plugin for HTTP and hook execution. This count splits between two channels:
...
A separate UI VM pool (`plugin_max_ui_vms`) handles TUI screens and field interfaces. See the next section.

**Tuning guidance:**
- For plugins with light HTTP traffic and few hooks: `2` is sufficient.
- For plugins under heavy HTTP load: increase to `8-16`. Each VM consumes roughly 1-2 MB of memory.
...
Setting this to `0` means hooks compete with HTTP requests for the same VM pool. This works for plugins that do not register hooks, but risks hook starvation under HTTP load for plugins that do.
...
If a before-hook exceeds this timeout, it counts as an error: the hook's circuit breaker increments, and the transaction aborts.
...
Maximum total time for the entire before-hook chain on a single event. Multiple plugins can register before-hooks on the same event and table. This timeout caps the total time spent running all of them.

If the chain exceeds this timeout, remaining hooks are skipped and the transaction aborts.
...
The rate applies per-IP across all plugins, not per-plugin. Inactive entries are cleaned up automatically.
...
CIDR ranges for trusted reverse proxies. When a request arrives from a trusted proxy IP, ModulaCMS parses the `X-Forwarded-For` header to extract the real client IP for rate limiting and `req.client_ip`.
...
When the circuit breaker opens, all HTTP requests to the plugin return 503 until the reset interval elapses.
...
Consecutive errors before a hook-level circuit breaker trips. Each (plugin, event, table) combination has its own breaker. When tripped, that specific hook is disabled until you reload or re-enable the plugin.
